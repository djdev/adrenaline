...
export default class InputField extends Component {
  ...
  render() {
    const { placeholder, submitLabel, onSubmitSuggested, suggestedMessages } =
      this.props;
    const { value } = this.state;
    ...
    return (
      <div id="inputField">
          ...
          <div id="suggestedMessages">
            {suggestedMessages.map((suggestedMessage, index) => {
              return (
                <motion.div
                  initial={{
                    translateY: 50 * (suggestedMessages.length - index),
                    opacity: 0.0,
                  }}
        ...
        <div id="inputFieldArea">
          ...
          />
          <Button id="sendInputButton" isPrimary onClick={this.onSubmitInput}>
            {submitLabel}
          </Button>
        </div>
      </div>
