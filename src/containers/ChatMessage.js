...
export default class ChatMessage extends Component {
  constructor(props) {
    ...
    this.renderSuggestChangesButton =
      this.renderSuggestChangesButton.bind(this);
    ...
    if (!isUserSubmitted && isComplete && containsCode) {
      return (
        <Button
          ...
          isPrimary
        >
          <img src="./integrate_icon.png" alt="" />
          Suggest changes
        </Button>
      );
    }
  ...
  renderRegenerateButton() {
    const { isComplete, isLastMessage, isUserSubmitted, onRegenerateResponse } =
      this.props;

    if (!isUserSubmitted && isComplete && isLastMessage) {
      return (
        <div id="regenerateResponse" onClick={onRegenerateResponse}>
          <img src="./regenerate_icon.png" alt="" />
        </div>
      );
    }
  }
  ...
  render() {
    ...
    const shouldRenderSuggestChanges =
      !isUserSubmitted && isComplete && containsCode;

    return (
      <div
        className={`chatMessage ${!isUserSubmitted ? "aiResponse" : ""} ${
