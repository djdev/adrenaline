...
class ChatBot extends Component {
  constructor(props) {
    ...
    this.state = {
      messages: [
        {
          message:
            "I'm an AI assistant that can help debug your code. What seems to be the problem?",
  ...
  onSendMessage(message, isRegeneration = false, isSuggested = false) {
    ...
    const { code, clearSuggestedMessages, errorMessage, shouldUpdateContext } =
      this.props;
    const { messages } = this.state;
    ...
    if (window.location.protocol === "https:") {
      this.ws = new WebSocket(
        `wss://rubrick-api-production.up.railway.app/generate_chat_response`
      );
    } else {
      this.ws = new WebSocket(
        `ws://rubrick-api-production.up.railway.app/generate_chat_response`
      );
    }
    ...
    this.ws.onmessage = (event) => {
      const { message, document_ids, is_rate_limit_error } = JSON.parse(
        event.data
      );
      const { messages } = this.state;
      ...
      this.setState({
        messages: [...messages.splice(0, messages.length - 1), response],
      });
    };
  ...
  onRegenerateResponse() {
    ...
    this.setState({
      messages: [
        ...
        { message: "", isUserSubmitted: false, isLoading: true },
      ],
    });
  ...
  renderChatMessages() {
    ...
    return messages.map((messagePayload, index) => {
      const { isUserSubmitted, message, isComplete, isBlocked, isLoading } =
        messagePayload;
